...
function loadData() {
    ...
    if (savedMaterials) {
        try { const parsed = JSON.parse(savedMaterials); if (Array.isArray(parsed)) materials.splice(0, materials.length, ...parsed); }
        catch(e){ console.error(e); }
    }

    if (savedPlans) {
        try { const parsed = JSON.parse(savedPlans); if (parsed && typeof parsed === "object") Object.assign(dailyPlans, parsed); }
        catch(e){ console.error(e); }
    }
}
...
function addTapToggle(itemDiv) {
    itemDiv.addEventListener("click", (e) => {
        // ボタン自身をクリックしたときは親のtoggleを実行しない
        if(e.target.closest("button")) return;
        itemDiv.classList.toggle("active"); // タップで表示・非表示切替
    });
}
...
function renderTodayPlans() {
    ...
    // 1. checked=false が上、checked=true が下
    // 2. 同じチェック状態なら time の昇順
    const sortedPlans = [...todayPlans].sort((a, b) => {
        ...
        return a.time.localeCompare(b.time); // HH:MM 形式で文字列比較でOK
    });
    ...
    sortedPlans.forEach((plan, index) => {
        ...
        if (plan.checked) {
            item.style.backgroundColor = "#f0f0f0"; // 薄い灰色
        }

        const iconDiv = document.createElement("div");
        iconDiv.className = "study-icon";
        iconDiv.innerHTML = '<i class="fa-solid fa-bookmark"></i>';
        if (plan.checked) iconDiv.querySelector("i").style.color = "#808080"; // 灰色
        ...
        checkBtn.style.color = plan.checked ? "#808080" : "green"; // チェック済みは灰色
        ...
        delBtn.style.color = plan.checked ? "#808080" : "#ff0000";
...
addMaterialModal.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
        e.preventDefault(); // デフォルトの改行や送信を防ぐ
        confirmAdd.click();
    }
});

addPlanModal.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
        e.preventDefault();
        confirmPlan.click();
    }
